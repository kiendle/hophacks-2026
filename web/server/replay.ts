...
export function replayPlugin(): Plugin {
  ...
  const load = () => dataset ??= readFile(new URL('../data/demo.json.gz', import.meta.url))
    .then((bytes) => JSON.parse(gunzipSync(bytes).toString()) as ReplayDataset)
    ...
    wss.on('connection', async (ws) => {
      ...
      try {
        ...
        const flush = () => {
          ...
          send({ type: 'batch', run, sequence: sequence++, now: clock.now, events, status: clock.status, speed: clock.speed })
        }
        restart()
        flush()
