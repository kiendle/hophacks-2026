...
export function TranslatablePost({ text, className, onInteract }: { text: string; className: string; onInteract?: () => void }) {
  // A changed post/text gets its own request state, even when the chart refreshes in place.
  return <PostText key={text} text={text} className={className} onInteract={onInteract} />
}

function PostText({ text, className, onInteract }: { text: string; className: string; onInteract?: () => void }) {
  ...
  return <>
    <p className={className} lang={english ? 'en' : undefined}>{english && result ? result.translated_text : text}</p>
