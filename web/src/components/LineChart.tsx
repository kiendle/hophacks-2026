...
function buildLayer(series: Series, view: TimeRange, cutoff: number): Layer {
  const points: Point[] = []
  const bs = series.buckets
  for (let i = 0; i < bs.length; i++) {
    if (bs[i].start > cutoff) break
    const t = Math.min(bs[i].start + BUCKET_MS, cutoff)
    ...
    points.push({ t, s: bs[i].sentiment, v: bs[i].volume, bucket: bs[i], grow: 1 })
  }
  return { series, points, tail: null }
}
...
export function LineChart({
  ...
  const layers = useMemo(
    () => series.map((s) => buildLayer(s, view, cutoff)),
    [series, view, cutoff],
  )
  ...
  const labels = layers
    .map((l) => {
      const inView = l.points.filter((p) => p.t <= view.end && Number.isFinite(p.s))
